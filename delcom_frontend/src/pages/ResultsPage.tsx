...
function ResultsPage() {
  ...
  if (result && (result as any).data.error === "please enter a valid address and a sensible craving") {
    console.log("error!");
    // bring up pop up with button that says go to home page
    return (
      <ErrorPopup message={(result as any).data.error} />    )
  }

  return (


    <div style={{padding: "40px",textAlign: "center",fontFamily: "Poppins", alignItems:"center"}}>
      <h1 style={{color:"white"}}>Results</h1>
      <div style={{display: "grid",gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))",gap: "20px",marginTop: "20px", marginLeft:"auto",marginRight:"auto"}}>
        ...
        {result?.data?.map((item, index) => (
          <BasicCard
            ...
            meta_rating={`Rating: ${item.meta?.rating ?? "N/A"}/5`}
            buttonText="View on UberEats"
            onButtonClick={() => window.open(`https://www.ubereats.com${item.url_info.action_url}?diningMode=DELIVERY&mod=quickView&modctx=%257B%2522storeUuid%2522%253A%2522${item.url_info.store_uuid}%2522%252C%2522sectionUuid%2522%253A%2522${item.url_info.section_uuid}%2522%252C%2522subsectionUuid%2522%253A%2522${item.url_info.subsection_uuid}%2522%252C%2522itemUuid%2522%253A%2522${item.url_info.item_uuid}%2522%252C%2522showSeeDetailsCTA%2522%253Atrue%257D&ps=1&surfaceName=`, "_blank")}
        ...
        { /*info popup logic
          /*the && here has the purpose of making sure the popup renders ONLY IF selectedItem exists, 
          hopfully thats never an issue because it shoudl always exist gulp*/}
      {selectedItem && ( 
  <div style={styles.popup}>
    <div style={{ ...styles.popupContent, display: "flex", gap: "20px", alignItems: "flex-start" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "8px" }}>
        ...
        {selectedItem.item_desc && <p style={styles.paragraph}>{selectedItem.item_desc}</p>}
    ...
    {selectedItem.customizations.split(";").map((group, index) => {
      ...
      return (
        <span key={index}>
          ...
          {index < selectedItem.customizations.split(";").length - 1 && <br />}
        </span>
      );
    })}
  </p>
        ...
        {selectedItem.meta && (
          <p style={styles.paragraph}>
            Price: ${selectedItem.meta.price ?? "N/A"} | ETA: {selectedItem.meta.eta ?? "N/A"} mins | Rating: {selectedItem.meta.rating ?? "N/A"}/5
          </p>
        )}
        <button
